...
{{ config(
    ...
    post_hook = "ALTER TABLE {{ this }} ADD SEARCH OPTIMIZATION ON EQUALITY(hash,parent_hash,receipts_root,sha3_uncles,state_root,transactions_root), SUBSTRING(hash,parent_hash,receipts_root,sha3_uncles,state_root,transactions_root)",
    full_refresh = false
) }}
